...
async function connectTarget(dbmsid) {
...
async function getSchemas(dbmsid) {
  let connection;
  try {
...
async function getTables(dbmsid, owner) {
  let connection;
  try {
    connection = await connectTarget(dbmsid);
...
async function getTableSpec(dbmsid, owner, tableNames) {
  let connection;
  try {
    connection = await connectTarget(dbmsid);

    const binds = tableNames.reduce((acc, name, idx) => {
      acc[`t${idx}`] = name;
      return acc;
    }, { owner });
    ...
    const searchConditionResult = await connection.execute(
    ...
    const searchConditionByConstraint = {};
    ...
    const grantsResult = await connection.execute(
    ...
    const synonymsResult = await connection.execute(
